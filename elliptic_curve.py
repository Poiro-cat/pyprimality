...
import numpy
import random
from .utils import gcd, exgcd, Jacobi, solve_quadratic_congruence
...
def reduced_class_polynomial(D:int):
    ...
    for i in range(degree+1):
        ...
        else: raise ArithmeticError('Non-Integer coefficient occurs in reduced class polynomial ...')
    return Polynomial(coef)

def complex_multiplication(N:int, D:int, RCP:Polynomial, u_odd:bool):
    if D == 1: return 1,0
...
class Point():
    def __init__(self, x:int, y:int=0):
        ...
        self.y = y
    def __eq__(self, p):
        if type(p) == int and p == 0: return self.x == self.y == 0
        if type(p) == Point: return self.x == p.x and self.y == p.y
        raise TypeError('Point must compare with another point or zero.')
    def __ne__(self, p):
        return not self == p
    def __str__(self):
        if self == 0: return '0'
...
class Elliptic_curve():
    def __init__(self, N:int, a:int, b:int):
        ...
        self.a = a
        self.b = b
    def point_on_curve(self, x:int, y:int):
        return (y*y - x**3 - self.a*x - self.b) % self.N == 0
    def random_point(self, zero_point:bool=True):          # if zero_point is true, points on x-axis are acceptable
        while True:
            x = random.randint(0, self.N-1)
            y2 = (x**3 + self.a*x + self.b) % self.N
            if y2 == 0:
                if zero_point: return Point(x,0)
                continue
            jacobi = Jacobi(y2, self.N)
            if jacobi == -1: continue
            if jacobi == 0: raise ValueError(gcd(y2, self.N))            # nontrivial factor of N is found
            ys = solve_quadratic_congruence(self.N, y2)
            return Point(x, list(ys)[random.randint(0,1)])
    def plus(self, P:Point, Q:Point):
        if P == 0: return Q    # tuple (0,) represents infinity point
        if Q == 0: return P
        if P.x != Q.x:
            c,inv_dx,_ = exgcd(P.x-Q.x, self.N)
            if c > 1: raise ValueError(c)                    # nontrivial factor of N is found
            k = (P.y-Q.y) * inv_dx % self.N
            x = (k*k-P.x-Q.x) % self.N
            y = (-k*(x-P.x)-P.y) % self.N
            if not self.point_on_curve(x,y): raise ArithmeticError('Group operation failed: point not on the curve')
            return Point(x,y)
        if (P.y + Q.y) % self.N == 0: return Point(0)
        if P.y == Q.y:
            c, inv_2y, _ = exgcd(2*P.y, self.N)
            if c > 1: raise ValueError(c)   # nontrivial factor of N is found
            k = ((3*P.x*P.x+self.a)*inv_2y) % self.N
            x = (k*k-2*P.x) % self.N
            y = (-k*(x-P.x)-P.y) % self.N
            if not self.point_on_curve(x,y): raise ArithmeticError('Group operation failed: point not on the curve')
            return Point(x,y)
        raise ArithmeticError('Not a group: x1==x2 but y1!=y2 and y1+y2!=0')
    def times(self, m:int, P:Point):     # an integer m multiples a point P
        Q = Point(0)
        while True:
            ...
            if pcd.degree:
                ...
                while tau <= (m+1)//2:
                    ...
                    if pcd.degree:
                        ...
                        break
                    tau += 1
                if tau > (m+1)//2: raise ArithmeticError('Frobenius endomorphism failed.')   # information that N is composite
            _, inv_M, inv_m = exgcd(M,m)
            t = (tm*M*inv_M+t*m*inv_m) % (m*M)
            M *= m
        if t >= M//2: t -= M
        if t*t > 4*self.N: raise ArithmeticError('Hasse theorem failed.')      # information that N is composite
        return self.N+1-t
